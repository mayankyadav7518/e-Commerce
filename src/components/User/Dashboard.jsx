...
export default function UserDashboard() {
  return (
    <div className="bg-gray-50 min-h-screen pb-12">
        ...
        <div className="max-w-7xl mx-auto grid lg:grid-cols-2 gap-8 lg:gap-10 items-center">
          <div>
            ...
            <div className="mt-6 flex flex-wrap gap-4">
              <button className="px-6 py-3 bg-teal-600 text-white rounded-xl font-medium hover:bg-teal-700 transition shadow-sm">
                Start shopping →
              </button>
              <button className="px-6 py-3 bg-white border border-gray-300 rounded-xl text-gray-700 hover:bg-gray-100 transition">
                Today’s best deals
              </button>
            </div>
          </div>
      ...
      <section className="px-4 sm:px-6 lg:px-8 py-6 max-w-7xl mx-auto">
        <div className="grid grid-cols-2 sm:grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4">
          {[
            ...
            { icon: Heart, label: "Wishlist", path: "/user/wishlist", color: "red" },
            ...
            <Link
              key={item.label}
              to={item.path}
              className="bg-white rounded-xl p-4 sm:p-5 shadow-sm hover:shadow-md hover:border-teal-200 border transition-all text-center group flex flex-col items-center"
            >
              <item.icon 
                size={32} 
                className={`mb-3 ${item.color === "red" ? "text-red-500" : "text-teal-600"} group-hover:scale-110 transition-transform`} 
              />
      ...
      {/* Flash / Mega Sale Banner (Meesho style urgency) */}
      <section className="px-4 sm:px-6 lg:px-8 py-6">
        <div className="max-w-7xl mx-auto bg-linear-to-r from-teal-600 to-emerald-600 rounded-2xl p-6 sm:p-10 text-white text-center shadow-2xl">
          <h2 className="text-2xl sm:text-4xl font-extrabold mb-2 tracking-tight">
            MEGA SAVINGS – ENDS TONIGHT!
          </h2>
          <p className="text-lg sm:text-xl mb-5 opacity-90">
            Up to 80% OFF + Extra ₹200 off on first order
          </p>
          <Link
            to="/user/men-products"
            className="inline-flex items-center px-8 py-4 bg-white text-teal-600 font-bold rounded-xl hover:bg-gray-100 transition-colors shadow-lg text-base"
          >
            Shop Mega Deals <ArrowRight className="ml-2" size={20} />
          </Link>
        </div>
      </section>
      ...
      <section className="px-4 sm:px-6 lg:px-8 py-8 max-w-7xl mx-auto">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-3 sm:gap-4">
          {trendingCategories.map((cat) => (
            <Link
              key={cat.name}
              to={`/user/${cat.name.toLowerCase().replace(/ /g, "-")}-products`}
        ...
        <div className="max-w-7xl mx-auto bg-linear-to-r from-purple-600 to-indigo-600 rounded-2xl p-6 sm:p-10 text-white shadow-2xl">
          ...
          <div className="text-center mb-8">
            ...
            <p className="text-base sm:text-lg text-white/90">
              Save more with exclusive deals, referral rewards, and instant discounts
            </p>
          </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5 mb-8">
            ...
            {/* Offer 1 */}
            <div className="bg-white/15 backdrop-blur-md rounded-xl p-5">
              <h3 className="font-semibold text-lg mb-1">🎁 Welcome Offer</h3>
              <p className="text-sm text-white/90 mb-3">
                Flat <span className="font-bold">₹300 OFF</span> on your first order.
              </p>
              <span className="inline-block bg-white/20 px-3 py-1 rounded-md text-sm font-mono">
                NEW300
              </span>
            </div>
            ...
            {/* Offer 2 */}
            <div className="bg-white/15 backdrop-blur-md rounded-xl p-5">
              <h3 className="font-semibold text-lg mb-1">🔥 Festive Sale</h3>
              <p className="text-sm text-white/90 mb-3">
                Get <span className="font-bold">20% OFF</span> on orders above ₹1999.
              </p>
              <span className="inline-block bg-white/20 px-3 py-1 rounded-md text-sm font-mono">
                FESTIVE20
              </span>
            ...
            <div className="bg-white/15 backdrop-blur-md rounded-xl p-5">
              ...
              <span className="inline-block bg-white/20 px-3 py-1 rounded-md text-sm">
                Auto Applied
              </span>
            </div>
          </div>
